...
RC GroupByVecPhysicalOperator::next(Chunk &chunk)
{
  ...
  for (int i = 0; i < tuple.cell_num(); ++i) {
    ...
    auto column = make_unique<Column>(value.attr_type(), value.length());
    column->append_one((char*)value.data());
...
RC GroupByVecPhysicalOperator::find_group(const Tuple &child_tuple, GroupType *&found_group)
{
  found_group = nullptr;
